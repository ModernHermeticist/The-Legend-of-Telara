...
def choose_easy_monster(monster_choice, dungeon_level, x, y):
	if monster_choice == 'gnoll':
		class_component = Fighter(hp=20+(dungeon_level), mp=0, defense=0, min_damage=0,
									max_damage=3+(dungeon_level-1), xp=20+(2*(dungeon_level-1)))
		...
		class_component = Fighter(hp=20+(2*dungeon_level-1), mp=0, defense=0, min_damage=0,
									max_damage=4+(dungeon_level-1), xp=35+(5*(dungeon_level-1)))
		ai_component = BasicMonster()

		monster = Entity(x, y, 'o', libtcod.desaturated_green, 'Orc', alive=True, blocks=True,
						 render_order=RenderOrder.ACTOR, combat_class=class_component, ai=ai_component)
	elif monster_choice == 'troll':
		class_component = Fighter(hp=30+(2*dungeon_level-1), mp=0, defense=3, min_damage=0,
									max_damage=6+(dungeon_level-1), xp=100+(5*(dungeon_level-1)))
		ai_component = BasicMonster()
